...
library.createWindow = 
	function(skipLoader)
		...
		WindowFunctions.createTab = function(tabName, defaultTab)
			...
			TabFunctions.createSlider = function(name, icon, min, max, default, floor, callback)
				min = min or 0
				max = max or 10
				...
				SliderFunctions.move = function()
					...
					valuee = Value
					if floor then
						Value = tonumber(math.floor(Info.Minimum + ((Info.Maximum - Info.Minimum) * Px)))
					else
				...
				DropdownFunctions.AddValue = function(value)
					...
					ValueFunctions.Enable = function()
						...
						if multi_selection then
							if not table.find(DropdownFunctions.Values,value) then
					...
					InvisibleButton.MouseButton1Down:Connect(function()
						if not ValueFunctions.Enabled then
			...
			TabFunctions.createAdditionalTab = function()
				...
				AdditionalTabFunctions.createSlider = function(name, icon, min, max, default, floor, callback)
					min = min or 0
					max = max or 10
